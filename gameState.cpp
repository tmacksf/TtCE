...
void gameState::initialise(std::string fen) {
    ...
    for (i = 0; i < fenSeperated[2].length(); i++){
        if (fenSeperated[2][i] == 'K') m_castling[0] = true;
        if (fenSeperated[2][i] == 'Q') m_castling[1] = true;
        if (fenSeperated[2][i] == 'k') m_castling[2] = true;
        if (fenSeperated[2][i] == 'q') m_castling[3] = true;
    }
...
void gameState::printing() {
    ...
    for (int i = 63; i >= 0; i--) {
        int flag = 0;
        for (int j = 0; j < 12; j ++) {
            if (this->m_bitboards[j].getBitAt(i) == 1) {
...
void gameState::makeMove(const Move &move) {
    ...
    // white castling priv
    if (move.piece == ROOK) {
        if (move.fromSquare == h1) m_castling[0] = false;
        else if (move.fromSquare == a1) m_castling[1] = false;
    }
    // black castling priv
    if (move.piece == ROOK+6) {
        if (move.fromSquare == h8) m_castling[2] = false;
        else if (move.fromSquare == a8) m_castling[3] = false;
    }

    if (move.piece == K) m_castling[0] = false, m_castling[1] = false;
    if (move.piece == k) m_castling[2] = false, m_castling[3] = false;
    if (move.castleFlag) {
...
void gameState::stateToFen() {
    ...
    if (m_castling[0]) output += 'K';
    if (m_castling[1]) output += 'Q';
    if (m_castling[2]) output += 'k';
    if (m_castling[3]) output += 'q';
    output += ' ';
    (m_enPassantSquare != -1) ? output += boardMap[m_enPassantSquare] : output+= '-';
    output += " 0 1";
