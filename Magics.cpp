...
BB Magics::randomBB() {
    BB u1, u2, u3, u4;
    u1 = (BB)(random()) & 0xFFFF; u2 = (BB)(random()) & 0xFFFF;
    u3 = (BB)(random()) & 0xFFFF; u4 = (BB)(random()) & 0xFFFF;
    return u1 | (u2 << 16) | (u3 << 32) | (u4 << 48);
}

BB Magics::randomBBFewBits() {
    return randomBB() & randomBB() & randomBB();
}
...
BB Magics::findRookMagic(int sq, int numBits) {
    ...
    for(randomCount = 0; randomCount < 100000000; randomCount++) {
        ...
        if(Bitboard::bitCount((attackMask * magic) & 0xFF00000000000000ULL) < 6) continue;
...
void Magics::initRookAttacks() {
    ...
    for (int square = 0; square < 64; square ++) {
        ...
        for (int i = 0; i < occupancyIndices; i++) {
            ...
            rookAttacks[square][(occ * preCalcRookMagics[square]) >> (64 - rookBitCounts[square])] = rookAttacksOnTheFly(square, occ);
        }
    }
}
...
void Magics::initBishopAttacks() {
    for (int square = 0; square < 64; square ++) {
        ...
        for (int i = 0; i < occupancyIndices; i ++) {
            BB occ = setOccupancy(i, mask);
            bishopAttacks[square][(occ * preCalcBishopMagics[square]) >> (64 - bishopBitCounts[square])] = bishopAttacksOnTheFly(square, occ);
        }
    }
}
...
void Magics::generateSlidingAttackTables() {
    ...
    // generate magics for rooks and bishops (already have precalculated magics)
